...
###	latter not for pacbio): error rates R binary object 4. taxa_SV.tsv = taxonomic annotaitons 5. track_reads.txt & track_read_rel.txt: summary on per-step remaining sequences
#### PSEUDOCODE:
###	1. parse arguments and set profiles 2. optional: remove primer sequences with
###		a. cutadapt in V3V4 and Archaea b. R function in Pacbio c. no removal V1V2, Fungi 3. filtering of data 4. estimation of error rates for fwd and rev reads 5. error correction for fwd and rev reads 6. merging of reads 
###	7. chimera removal 8. taxonomic annotation 9. collect statistics
...
## Set number of CPUS
if(is.na(args[3])){ threads=args[3] }else{threads=24}
### settings for different profiles V3V4 and Archaea require trimming of primer sequences by cutadapt; sequences are given by FWD and REV variables
if(profile=="V1V2"){
	truncLens=c(230,180)
	trims=c(5,5)
	maxEEs=c(2,2)
...
##################################################################################
###                                                                     #########
###     	FROM HERE THE SAME FOR ALL PROFILES AGAIN ########
###			(not PACBIO, that ones finished)
###
###                                                                     ########
#################################################################################
###########################################
...
###########################################
#pdf(paste0(outdir,"/plots/plotQualityProfile.pdf"), onefile=T) capture.output(plotQualityProfile(fnFs[1:2])) capture.output(plotQualityProfile(fnRs[1:2])) dev.off()
##################################
