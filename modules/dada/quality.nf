process DADA2_QUALITY {
    ...
    output:
    path "${projects}_qual_stats.pdf"            , emit: pdf
    ...
    if ( sum_readfiles_length > 2147483647 ) {
        ...
        write.table(max_files, file = paste0(max_files," files were used for ${project} plotQualityProfile.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE, na = '')
    }
    ...
    #write output
    write.table( t(df), file = paste0("${project}_qual_stats",".tsv"), sep = "\t", row.names = TRUE, col.names = FALSE, quote = FALSE)
    pdf(paste0("${project}_qual_stats",".pdf"))
    plot
    dev.off()
